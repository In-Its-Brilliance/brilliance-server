...
use crate::network::{events::on_connection_info::PlayerConnectionInfoEvent, server::NetworkPlugin};
...
pub struct Client {
    ...
    /// Server-side persistent player data loaded after login and saved on disconnect.
    player_data: Arc<RwLock<Option<PlayerData>>>,
...
impl Client {
    ...
    pub fn get_player_data(&self) -> Option<MappedRwLockReadGuard<'_, parking_lot::RawRwLock, PlayerData>> {
        RwLockReadGuard::try_map(self.player_data.read(), |p| match p {
            Some(data) => Some(data),
            None => None,
        })
        .ok()
    ...
    pub fn read_player_data(&self, storage: &ServerStorageManager) -> Result<(), String> {
        ...
        let player_data = match storage.get_or_create_player_data(login.clone()) {
            Ok(player_data) => player_data,
            Err(e) => {
                return Err(e);
            }
        };
        *self.player_data.write() = Some(player_data);
        Ok(())
    }

    pub fn save_player_data(&self, storage: &ServerStorageManager) -> Result<(), String> {
        let Some(player_data) = self.get_player_data() else {
            return Ok(());
        };

        storage.save_player_data(&player_data).map(|_| ())
    }
...
impl ConsoleSenderType for Client {
